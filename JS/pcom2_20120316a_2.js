    ...
    /*    
      //	 this is to try to build it from scratch	  
        ...
        */
           ...
           if(fixInt(objForm.FFBQuantity.value) > 0) 
				{
...
// Commented 2009 doesn't have all the discounts	if(a == 3 && (isCompDinnerDance(objForm.classYear) || is2000(objForm.classYear) ||isBelow1957(objForm.classYear) )) 
					...
					if(a == 0 && (isCompDinnerDance(objForm.classYear) )) 					
  					{
  					 if(fixInt(objForm.FFBQuantity.value) > 2) 
  						{
				...
				switch(a) 
				{
					...
					case 0:
						...
						if (fixInt(objForm.FFBQuantity) > 2) intMaxDiscount = 2;
						else intMaxDiscount = fixInt(objForm.FFBQuantity);
						intFFBSubtotal = fixInt(objForm.FFBQuantity.value) * intCost;
...
//mw start			  
			  ...
			  objForm.FFBTotal.value = fixInt(objForm.FFBQuantity) * fixInt(objForm.FFBItemCost);
...
//              total = total   fixInt(objForm.Total3);    
          ...
          case 2:	
					...
					case 3:
						...
						//  moved below objForm.RDDTotal.value = (fixInt(objForm.RDDItemCost) *	fixInt(objForm.RDDQuantity));
            // moved below total = total   fixInt(objForm.RDDTotal);
						break;
					...
					//objForm.CBTotal.value = fixInt(objForm.Quantity[a]) * fixInt(objForm.ItemCost[a]);
						if(isNaN(objForm.CBTotal.value)){
							...
							objForm.CBTotal.focus();
								...
								// moved below  total = total   fixInt(objForm.CBTotal);
...
//			document.getElementById("message").innerHTML = '? Complimentary Tickets for Class of 1962';
				...
				if(fixInt(objForm.FFBQuantity.value) == 1) {
					objForm.FFBTotal.value = (fixInt(objForm.FFBItemCost) *	fixInt(objForm.FFBQuantity));
					total = total   fixInt(objForm.FFBTotal);
